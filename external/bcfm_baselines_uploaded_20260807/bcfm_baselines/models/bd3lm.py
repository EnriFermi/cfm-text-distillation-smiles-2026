...
# Rejection-sampling budget for the per-block masked fraction.
RESAMPLE_ATTEMPTS = 128
...
class BlockDiffusionTextModel(GenerativeTextModel):
    ...
    def _mask_exact_counts(self, valid_blocks: Tensor, counts: Tensor) -> Tensor:
        """Mask exactly ``counts`` uniformly chosen valid positions per block."""
        priorities = torch.rand(valid_blocks.shape, device=valid_blocks.device)
        priorities = priorities.masked_fill(~valid_blocks, float("inf"))
        ranks = priorities.argsort(-1).argsort(-1)
    ...
    def _sample_mask(
        ...
        # Rejection sampling can still miss when a block's rate sits against its
        # bound.  Nudging those few blocks onto the nearest admissible count is
        # the only option that keeps a long training run alive.
        counts = masked_blocks.sum(-1)
        invalid = (counts < minimum_count) | (counts > maximum_count)
        nudged = self._mask_exact_counts(
            valid_blocks, counts.clamp(min=minimum_count, max=maximum_count)
        )
        masked_blocks = torch.where(invalid[..., None], nudged, masked_blocks)
        return masked_blocks.reshape_as(masked)
    ...
    def compute_loss(
        ...
        **kwargs,
        ...
        loss = (weighted * valid).sum() / valid.sum().clamp_min(1)
        ...
        return {
            ...
            "masked_fraction": masked.float().sum().detach() / valid.sum().clamp_min(1),
            "token_count": valid.sum().detach(),
    ...
    def generate(self, generation_config: GenerationConfig, **kwargs) -> GenerationResult:
        ...
        steps = config.steps_per_block or config.num_steps
        if steps <= 0:
        ...
        for _ in range(config.length // self.block_size):
            ...
            if config.first_hitting:
                ...
                # position in each sequence.  A block is complete after B NFEs.
                ...
                for _ in range(steps):
                    ...
                    proposals, probabilities = sample_logits(logits, config, generator)
