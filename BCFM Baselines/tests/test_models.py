...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("configs/tinystories/bd3lm_b4.yaml", 4),
        ("configs/tinystories/bd3lm_b8.yaml", 8),
        ("configs/tinystories/bd3lm_b16.yaml", 16),
        ("configs/tinystories/bd3lm_b32.yaml", 32),
    ],
)
def test_research_bd3_configs_use_paper_style_initialization_and_sampler(
    ...
    assert config["training"]["ema_decay"] == pytest.approx(0.9999)
    assert config["training"]["require_pretrained_ema"] is True
    assert config["training"]["schedule_search_start_step"] == 5000
    assert config["generative_evaluation"]["first_hitting"] is False
    assert (
        256 // block_size * config["generative_evaluation"]["steps_per_block"]
        == 64
    )
...
def test_tinystories_ar_matches_cfm_backbone_and_training_budget() -> None:
    config = load_config("configs/tinystories/ar.yaml")
    backbone = config["model"]["backbone"]
    assert config["dataset"]["tokenizer"] == "gpt2"
    assert config["dataset"]["vocabulary_size"] == 50257
    assert config["dataset"]["sequence_length"] == 256
    assert backbone == {
        "vocab_size": 50258,  # GPT-2 plus the AR-only context BOS.
    ...
    assert training["batch_size"] * training["gradient_accumulation_steps"] == 128
    assert training["max_steps"] == 100000
    assert training["learning_rate"] == pytest.approx(1e-4)
    assert training["weight_decay"] == pytest.approx(0.1)
    assert training["betas"] == [0.9, 0.999]
    assert training["epsilon"] == pytest.approx(1e-8)
    assert training["warmup_steps"] == 500
    assert training["warmup_start_factor"] == pytest.approx(1e-3)
    assert training["lr_decay"] == "constant"
    assert training["gradient_clip_norm"] == pytest.approx(1.0)
    assert training["precision"] == "bf16"
    assert training["validation_every"] == 5000
...
def test_bd3_float64_gumbel_race_matches_official_formula() -> None:
    logits = torch.tensor([[0.2, -0.1, 0.7]])
    config = GenerationConfig(length=1, batch_size=1, seed=19)
    generator = torch.Generator().manual_seed(19)
    actual, probabilities = sample_logits_gumbel64(logits, config, generator)

    expected_probabilities = logits.softmax(-1).to(torch.float64)
    ...
    expected = (
        expected_probabilities / (1e-10 - (uniform + 1e-10).log())
    ).argmax(-1)
...
def test_bd3_rejects_explicit_zero_steps_per_block() -> None:
    config = load_config("configs/smoke/text8_bd3lm.yaml")
    model = create_model(config).eval()
    with pytest.raises(ValueError, match="steps_per_block must be positive"):
        model.generate(
            GenerationConfig(**{**config["sampling"], "steps_per_block": 0})
        )
