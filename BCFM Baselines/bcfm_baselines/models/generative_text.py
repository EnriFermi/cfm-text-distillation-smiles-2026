...
def sample_logits_gumbel64(
    logits: Tensor,
    config: GenerationConfig,
    generator: torch.Generator,
) -> tuple[Tensor, Tensor]:
    """Paper-corrected BD3 categorical draw using a float64 exponential race.

    Appendix C.5 and the official implementation avoid ``multinomial`` here:
    low-precision categorical sampling measurably hurts generative perplexity.
    Logit filtering/normalization follows the regular path, then probabilities
    and the race noise are promoted to float64 before selecting the argmax.
    """
    ...
    # The official BD3 code applies nucleus truncation after converting p(x0)
    # to float64, then renormalizes. Preserve that order as well as its unusual
    # exclusion of the token that crosses the cumulative-p boundary.
    ...
    if config.strategy == "greedy":
        ...
        samples = (probabilities / exponential).argmax(dim=-1)
    return samples, probabilities
