...
Initially, block mask rates are stratified uniformly on `[epsilon, 1]`. At validation, the
trainer also evaluates the full interval and every width in
...
selects the minimum-variance interval. Identical RNG states are used across
candidates on a batch, avoiding a noise-induced comparison bias.
...
Training maintains EMA weights with decay `0.9999`; schedule search and
validation run on EMA weights, and inference loads the saved EMA state by
default. The selected schedule bounds are checkpointed and restored on resume.
...
of size `B` finishes in exactly `B` network evaluations per sequence. The
